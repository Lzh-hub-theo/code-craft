...
import com.yu.ai.yuaicodemother.langgraph4j.ai.CodeQualityCheckService;
        ...
        return node_async(state -> {
            ...
            try {
                ...
                if (StrUtil.isBlank(codeContent)) {
                    ...
                    qualityResult = codeQualityCheckService.checkQuality(codeContent);
                    log.info("代码质量检查完成 - 是否通过：{}", qualityResult.getIsValid());
                }
            } catch (Exception e) {
                log.error("代码质量检查失败: {}", e.getMessage(), e);
                qualityResult = QualityResult.builder()
                        .isValid(true)
                        .build();
            }
