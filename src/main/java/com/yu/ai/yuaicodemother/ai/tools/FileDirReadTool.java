package com.yu.ai.yuaicodemother.ai.tools;
...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(
        ...
        try {
            ...
            // 使用Hutool递归获取所有文件
            List<File> files = FileUtil.loopFiles(targetDir, file -> !shouldIgnore(file.getName()));
            // 按路径深度和名称排序显示
            files.stream()
                    .sorted((f1, f2) -> {
    ...
    private boolean shouldIgnore(String fileName) {
        ...
        // 检查文件扩展名符不符合要求 IGNORE_EXTENSIONS.stream().anyMatch(fileName::endsWith)
